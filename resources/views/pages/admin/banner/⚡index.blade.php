    ...
    <div class="shadow-md ring-1 ring-gray-200 rounded-md relative">
        ...
        >
            ...
            @scope('cell_title_2', $banner)
                ...
                <div class="text-xs text-gray-400 line-clamp-1">{{ $banner->getTranslation('description', 'en', false) ?: '' }}</div>
            @endscope
            ...
            @scope('cell_actions', $banner)
                <div class="flex gap-2">
                    <x-button icon="o-pencil" class="btn-xs btn-ghost text-primary" wire:click="openEdit({{ $banner->id }})"/>
    ...
    <x-modal wire:model="showCreate" title="Thêm banner" class="modalAddBanner" separator>
        <div class="space-y-1 py-0 max-h-[70vh] overflow-y-auto pr-1">
            <div class="space-y-2">
                ...
                >
                <p class="text-xs text-gray-500">Kích thước tối ưu: 1920x550px. Vùng an toàn: khoảng 1100x550px. Dung lượng tối đa: 5MB.</p>
                ...
                <div class="relative min-h-32 rounded-lg border border-dashed border-gray-300 bg-gray-50/70 overflow-hidden">
                    ...
                    <div wire:loading.remove wire:target="image" class="h-32 flex items-center justify-center">
                        @if($image)
                            <img src="{{ $image->temporaryUrl() }}" class="h-full w-full object-cover" alt="Ảnh xem trước"/>
                        @else
            ...
            <x-tabs wire:model="tab_create_select" active-class="text-fita! border-b-2 border-fita font-semibold">
                <x-tab name="vi" label="Tiếng Việt" class="mb-0 py-0!">
                    <x-input label="Tiêu đề 1 (Tiếng Việt)" wire:model.live.debounce.300ms="title_vi" placeholder="Nhập tiêu đề 1 tiếng Việt"/>
                    <x-input label="Tiêu đề 2 (Tiếng Việt)" wire:model.live.debounce.300ms="description_vi" placeholder="Nhập tiêu đề 2 tiếng Việt" class="col-span-full"/>
                    <x-input label="Nhãn nút (Tiếng Việt)" wire:model.live.debounce.300ms="url_text_vi" placeholder="Ví dụ: Xem thêm"/>
                </x-tab>
                <x-tab name="en" label="Tiếng Anh"  class="mb-0 py-0!">
                    <x-input label="Tiêu đề 1 (Tiếng Anh)" wire:model.live.debounce.300ms="title_en" placeholder="Nhập tiêu đề 1 tiếng Anh"/>
                    <x-input label="Tiêu đề 2 (Tiếng Anh)" wire:model.live.debounce.300ms="description_en" placeholder="Nhập tiêu đề 2 tiếng Anh" class="col-span-full"/>
                    <x-input label="Nhãn nút (Tiếng Anh)" wire:model.live.debounce.300ms="url_text_en" placeholder="Ví dụ: Xem thêm"/>
                </x-tab>
            </x-tabs>
            <div class="grid grid-cols-2 gap-y-1 gap-x-4">
                ...
                <div class="flex justify-end flex-col"><x-checkbox label="Hoạt động" wire:model="is_active" class="checkbox-primary"/></div>
            </div>
        </div>
                ...
                <div class="relative min-h-32 rounded-lg border border-dashed border-gray-300 bg-gray-50/70 overflow-hidden">
                    ...
                    <div wire:loading.remove wire:target="image" class="h-32 flex items-center justify-center">
                        ...
                        @elseif($oldImagePath && Storage::disk('public')->exists($oldImagePath))
                            <img src="{{ Storage::url($oldImagePath) }}" class="h-full w-full object-cover" alt="Ảnh hiện tại"/>
                        @else
                            <span class="text-sm text-gray-500">Chưa chọn ảnh</span>
                        @endif
                    </div>
