...
class extends Component {
    ...
    {
        $previewData = \Illuminate\Support\Facades\Cache::get('preview_footer_data');
...
{{--    </x-slot:breadcrumb>--}}
    ...
    @else
    <div class="bg-fita pt-10">
        <div
            class="flex w-full max-w-330 px-4 sm:px-6 lg:px-8 mx-auto py-8 gap-5 lg:gap-25 text-white text-[15px] font-barlow justify-between flex-wrap flex-col lg:flex-row">
            <div>
                ...
                <ul class="mt-5 space-y-2">
                    ...
                    @if(!empty($pageData['contact']['phone']))
                        <li class="hover:opacity-85">
                            @php
                                $phoneString = $pageData['contact']['phone'] ?? '';
                                $telLink = '';

                                if (preg_match('/[0-9\+\-\.\s\(\)]{8,20}/', $phoneString, $matches)) {
            ...
            <div class="flex-1 w-full hidden lg:block">
                <h5 class="text-[22px] font-semibold">{{__('Information')}}</h5>
                    ...
                    @foreach($pageData['quick_links'] as $index => $item)
                        @if(!empty(trim($item['name'])) && !empty(trim($item['url'])))
                            <li class="hover:scale-x-105">
                                <a href="{{$item['url']}}" class="flex items-center hover:underline">
